...
export interface UploadState {
  // In React Native, file objects may come from an image/document picker
  files: any[]; 
  previews: string[];
  caption: string;
  hashtags: string[];
...
export function useUploadState() {
  ...
  const autoSave = async () => {
    try {
      ...
      const draft = {
        files: state.files, // Ensure these files are in a format compatible with your storage solution
