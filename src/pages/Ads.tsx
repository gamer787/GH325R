...
// Ensure that this file exists and exports your Supabase client
...
export default function ViewSponsored(): React.JSX.Element {
  ...
  const [contentList, setContentList] = useState<SponsoredContent[]>([]);
  ...
  const loadContent = async () => {
    try {
      ...
      const transformed = data?.map((item: SponsoredContent) => ({
        ...item,
        applications_count: (item as any).applications?.[0]?.count || 0,
      })) || [];
  ...
  const handleStatusChange = async (contentId: string, newStatus: SponsoredContent['status']) => {
    try {
      ...
      if (error) throw error;
      setContentList(prev =>
        prev.map((item: SponsoredContent) =>
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (error) throw error;
      setContentList(prev => prev.filter((item: SponsoredContent) => item.id !== contentToDelete.id));
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {contentList.length === 0 ? (
        ...
        <View style={styles.contentList}>
          {contentList.map((item: SponsoredContent) => (
          ...
          <View style={styles.modalBackdrop}>
            <View style={styles.modalContainer}>
              ...
              <View style={styles.modalActions}>
                <TouchableOpacity
                  onPress={() => setContentToDelete(null)}
